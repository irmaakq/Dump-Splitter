    ...
    <div className={`fixed inset-0 z-[90] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-[#0f0f0f] border border-white/10 rounded-3xl max-w-3xl w-full p-8 relative shadow-2xl overflow-y-auto max-h-[85vh] custom-scrollbar transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Grid size={14} className="text-violet-400" /> PARÇA SAYISI (3, 4, 6, 9) NEDEN FARKLI BÖLÜNÜYOR?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>Çift Sayılar (2, 4, 6, 8, 10):</strong> Sistem bunları genellikle 'Izgara' (Grid) veya 'Eşit Bölme' mantığıyla işler (Örn: 4 parça 2x2, 6 parça 2x3 grid olabilir veya dikeyde 2 sıra olabilir). Bu, özellikle dikey fotoğraflarda kenarlarda boşluk oluşmasına neden olabilir.<br /><strong>Tek Sayılar (3, 5, 7, 9):</strong> Bu sayılar genellikle tek bir sütun halinde (alt alta) bölünür. Bu yapı, fotoğrafın orijinal en/boy oranını bozmadan ekranı tam doldurmasını sağlar ve kenarlarda boşluk kalmaz.</p></div>
