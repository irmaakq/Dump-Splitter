...
import {
  ...
  Download, Eye, Shield, Github, Settings, ChevronRight, Loader2, Menu, Trash2, RefreshCcw, Archive, Layers, Smartphone, Wand2, AlertTriangle, Cookie, Scale, MousePointerClick, ListChecks, Scissors, Files, Move, Minimize, Home, Timer, Eraser, Columns, FolderInput, Layout, ZoomOut // Added New Icons
} from 'lucide-react';
...
const FEATURE_DETAILS = {
  ...
  ultraHd4x: {
    ...
    shortDesc: "Profesyonel model ile çözünürlüğü 4 katına çıkarır.",
    desc: "Gelişmiş ESRGAN Thick modeli kullanarak görseli 4 kat büyütür. İşlem süresi daha uzundur ancak en yüksek detayı sağlar."
  }
};
    ...
    <div className={`fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 overflow-y-auto transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-[#0f0f0f] border border-white/10 rounded-3xl max-w-3xl w-full p-6 md:p-10 relative shadow-2xl my-auto transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors h-full"><h3 className="text-white font-bold mb-3 flex items-center gap-3"><Lock size={18} className="text-blue-400" /><span className="uppercase tracking-tight text-xs">İstemci Taraflı İşleme</span></h3><p className="text-gray-400 leading-relaxed text-xs">Dump Splitter, dosyalarınızı <strong>uzak bir sunucuya (Cloud) yüklemez.</strong> Tüm işlemler tarayıcınızın belleğinde (HTML5 Canvas) gerçekleşir. Verileriniz cihazınızdan asla dışarı çıkmaz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors h-full"><h3 className="text-white font-bold mb-3 flex items-center gap-3"><ServerOff size={18} className="text-red-400" /><span className="uppercase tracking-tight text-xs">Sunucu Kaydı Yoktur</span></h3><p className="text-gray-400 leading-relaxed text-xs">Uygulama sunucusuz çalışır. Fotoğraflarınız loglanmaz veya kaydedilmez. Sayfayı yenilediğinizde (F5), tüm geçici veriler RAM'den silinir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors h-full"><h3 className="text-white font-bold mb-3 flex items-center gap-3"><Eye size={18} className="text-purple-400" /><span className="uppercase tracking-tight text-xs">AI Modeli Eğitilmez</span></h3><p className="text-gray-400 leading-relaxed text-xs">Kullanılan algoritmalar yereldir. Görselleriniz, herhangi bir yapay zeka modelini eğitmek veya yüz taraması yapmak amacıyla <strong>kullanılmaz.</strong></p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors h-full"><h3 className="text-white font-bold mb-3 flex items-center gap-3"><Shield size={18} className="text-yellow-400" /><span className="uppercase tracking-tight text-xs">Sıfır İz Politikası</span></h3><p className="text-gray-400 leading-relaxed text-xs">Kişisel verileriniz, IP adresiniz veya kullanım alışkanlıklarınız hiçbir üçüncü taraf ile paylaşılmaz. Uygulama tamamen anonimdir.</p></div>
        </div>
        <div className="mt-6 bg-white/[0.03] p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors text-sm"><h3 className="text-white font-bold mb-3 flex items-center gap-3"><Cookie size={18} className="text-amber-500" /><span className="uppercase tracking-tight text-xs">Çerez (Cookie) Kullanımı</span></h3><p className="text-gray-400 leading-relaxed text-xs">Sitemizde sizi takip eden hiçbir çerez (cookie) veya reklam izleyicisi bulunmamaktadır. Tarayıcınızda sadece siteyi çalıştırmak için gerekli olan yerel veriler tutulur.</p></div>
  ...
  return (
    <div className={`fixed inset-0 z-[90] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-[#0f0f0f] border border-white/10 rounded-3xl max-w-4xl w-full p-8 md:p-10 relative shadow-2xl overflow-y-auto max-h-[90vh] transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <div className="block md:columns-2 gap-6 mb-10">
          <div className="bg-white/[0.03] border border-white/5 p-6 rounded-3xl hover:bg-white/[0.05] transition-colors group break-inside-avoid mb-6">
    ...
    <div className={`fixed inset-0 z-[90] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-[#0f0f0f] border border-white/10 rounded-3xl max-w-3xl w-full p-8 relative shadow-2xl overflow-y-auto max-h-[85vh] custom-scrollbar transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><FileQuestion size={14} className="text-yellow-400" /> Video yükleyebilir miyim?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet, video dosyalarını (MP4, MOV vb.) sisteme yükleyebilirsiniz. Ancak sistem videoları parça parça kesip video olarak vermez. Videonun o anki karesini <strong>yüksek kaliteli bir fotoğraf</strong> olarak yakalar ve bunu parçalara ayırır.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Lock size={14} className="text-green-400" /> Fotoğraflarım güvende mi?</h3><p className="text-gray-400 text-xs leading-relaxed">Kesinlikle. Sitemiz "Client-Side" (İstemci Taraflı) çalışır. Yüklediğiniz dosyalar sunucuya gönderilmez, sadece tarayıcınızın geçici hafızasında (RAM) işlenir. Sayfayı kapattığınız an her şey silinir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Monitor size={14} className="text-purple-400" /> Hangi cihazlarda çalışır?</h3><p className="text-gray-400 text-xs leading-relaxed">Dump Splitter; iPhone, Android, Tablet ve Bilgisayar (PC/Mac) tarayıcılarında sorunsuz çalışır. Herhangi bir uygulama indirmenize gerek yoktur.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Check size={14} className="text-blue-400" /> Ücretli mi, Sınır var mı?</h3><p className="text-gray-400 text-xs leading-relaxed">Tamamen ücretsizdir. Üyelik veya kredi sistemi yoktur. Performansın düşmemesi için aynı anda en fazla 20 dosya yükleyebilirsiniz ancak işlem bitince listeyi temizleyip tekrar yükleyebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><ImageIcon size={14} className="text-pink-400" /> Hangi formatlar destekleniyor?</h3><p className="text-gray-400 text-xs leading-relaxed">Giriş olarak JPG, PNG, WEBP, HEIC (tarayıcı desteğine göre) ve popüler video formatlarını kabul eder. Çıktı olarak PNG, JPG veya WEBP formatında indirebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Download size={14} className="text-orange-400" /> İndirme çalışmıyor?</h3><p className="text-gray-400 text-xs leading-relaxed">Eğer indirme başlamazsa tarayıcınızın "Pop-up engelleyicisini" kontrol edin veya sayfayı yenileyip (F5) tekrar deneyin. Tek tek indirmek için parçanın üzerindeki ok işaretine basmanız yeterlidir.</p></div>

          {/* YENİ EKLENEN SORULAR */}
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Sparkles size={14} className="text-fuchsia-400" /> AI Enhance renkleri bozar mı?</h3><p className="text-gray-400 text-xs leading-relaxed">Hayır, yapay zeka fotoğrafın doğal yapısını bozmadan sadece soluk renkleri, kontrastı ve doygunluğu optimize eder. Aşırı filtreli bir görünüm yerine profesyonel ve canlı bir sonuç verir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><DownloadCloud size={14} className="text-sky-400" /> Uygulamayı İndirebilir miyim?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet! Dump Splitter bir PWA (Progressive Web App) uygulamasıdır. Hem telefonunuza hem de bilgisayarınıza kurabilirsiniz.<br /><br /><strong>Telefonda:</strong> Tarayıcı menüsünden (Safari'de "Paylaş" &rarr; "Ana Ekrana Ekle", Chrome'da "Uygulamayı Yükle") diyerek.<br /><strong>Bilgisayarda:</strong> Chrome adres çubuğundaki yükle simgesine tıklayarak masaüstü uygulaması gibi kurabilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Target size={14} className="text-rose-400" /> Smart Crop fotoğrafımı keser mi?</h3><p className="text-gray-400 text-xs leading-relaxed">Smart Crop, fotoğrafın sadece en dışındaki %2'lik "ölü alanı" temizler. Bu işlem, ana objeyi merkeze daha iyi odaklamak ve Instagram'da daha profesyonel bir çerçeveleme sağlamak için yapılır.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Zap size={14} className="text-teal-400" /> Ultra HD modunda dosya boyutu artar mı?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet, Ultra HD modu fotoğrafın çözünürlüğünü 2 katına çıkardığı (Upscale) için dosya boyutu artabilir. Ancak bu, Instagram'ın sıkıştırma algoritmasına karşı fotoğrafınızın netliğini korumasını sağlar.</p></div>

          {/* YENİ AI ÖZELLİKLERİ HAKKINDA SORULAR (GÜNCEL) */}
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full">
          ...
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Menu size={14} className="text-indigo-400" /> ANA MENÜYE NASIL GİDİLİR?</h3><p className="text-gray-400 text-xs leading-relaxed">Sol üst köşedeki "D" logosuna veya "Dump Splitter" yazısına tıklayarak istediğiniz zaman ana başlangıç ekranına dönebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><RefreshCcw size={14} className="text-red-400" /> YENİ YÜKLEME NEDİR?</h3><p className="text-gray-400 text-xs leading-relaxed">"Yeni Yükleme" butonu, mevcut çalışma alanını tamamen temizler. Listede 20/20 fotoğraf olsa bile hepsi silinir ve sıfırdan başlamanızı sağlar.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Archive size={14} className="text-amber-400" /> TOPLU İNDİR BASINCA NEDEN BAZI İMAGELER İNMİYOR?</h3><p className="text-gray-400 text-xs leading-relaxed">Bazı dosya türleri toplu indirme (ZIP) sırasında teknik nedenlerle atlanabilir. Eğer bir fotoğraf inmezse, sol taraftaki listeden o fotoğrafın üzerine tıklayıp tekrar 'TÜMÜNÜ İNDİR' butonuna basabilir veya parçanın üzerindeki tekli indirme butonunu kullanarak indirebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Settings size={14} className="text-lime-400" /> EDİTÖRDEKİ ÖZELLİKLER (AI, HD, OPTIMIZE) NE İŞE YARAR?</h3><p className="text-gray-400 text-xs leading-relaxed">Bu özellikler fotoğrafınızı geliştirmenizi sağlar: <strong>AI Enhance</strong> renkleri canlandırır, <strong>HD Mode</strong> netliği artırır, <strong>Optimize</strong> boyutu küçültür, <strong>Smart Crop</strong> kadrajı düzeltir ve <strong>Ultra HD</strong> çözünürlüğü yükseltir. Her özelliğin yanındaki bilgi butonuna basarak daha detaylı açıklama görebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Columns size={14} className="text-indigo-500" /> NEDEN 3 SÜTUN (YAN YANA 3) YERİNE HEP 2 SÜTUN BÖLÜYOR?</h3><p className="text-gray-400 text-xs leading-relaxed">Instagram'da en iyi görünüm için 2 sütunlu yapı standarttır. Eğer 3 sütun (yan yana 3 parça) yapılsaydı, her bir parça "kürdan gibi" ince ve dar olurdu. Bu da fotoğrafın anlaşılmaz görünmesine neden olur. Bu yüzden sistem, görsel kalitesini korumak için 3 yerine 2 sütunlu (daha geniş parçalı) düzeni kullanır.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Activity size={14} className="text-cyan-400" /> HD & OPTIMIZE AÇINCA NEDEN FARK GÖREMİYORUM?</h3><p className="text-gray-400 text-xs leading-relaxed">Bu özellikler filtre değil, teknik kalite ayarlarıdır. <strong>HD Mode</strong> sıkıştırmayı kapatır (%100 Kalite), <strong>Optimize</strong> ise boyutu küçültür. Gözle görülür bir değişim (renk, netlik) istiyorsanız <strong>AI Enhance</strong> özelliğini açmalısınız.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><AlertTriangle size={14} className="text-red-500" /> HATA RAPORU NEDİR?</h3><p className="text-gray-400 text-xs leading-relaxed">Eğer indirme sırasında teknik bir sorun (hafıza dolması vb.) oluşursa, size bozuk dosya vermek yerine <strong>Hata_Raporu.txt</strong> dosyası verilir. Bu dosyayı açarak sorunun nedenini ve 'Tekrar Dene', 'Tekli İndir' gibi çözüm yollarını Türkçe olarak okuyabilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Smartphone size={14} className="text-emerald-400" /> İNDİRİLEN DOSYALAR NEREYE KAYDEDİLİYOR?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>iPhone (iOS):</strong> İndirilenler 'Fotoğraflar'a değil, mavi renkli <strong>'Dosyalar'</strong> uygulamasına kaydedilir. <strong>Android:</strong> Direkt 'Galeri' veya 'İndirilenler' klasörüne iner. Bildirim panelinden hemen ulaşabilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Scale size={14} className="text-gray-300" /> HİÇBİR AYAR SEÇMEZSEM KALİTE DÜŞER Mİ?</h3><p className="text-gray-400 text-xs leading-relaxed">Asla. Varsayılan mod, fotoğrafınızı "Dengeli ve Sağlıklı" bir kalitede (%95) ve orijinal çözünürlüğünde işler. Fotoğrafınız bozulmaz veya pikselleşmez. Ekstra ayarlar (HD, AI vb.) sadece özel iyileştirmeler içindir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><MousePointerClick size={14} className="text-stone-400" /> TEKLİ VEYA TOPLU İNDİRME ARASINDA KALİTE FARKI VAR MI?</h3><p className="text-gray-400 text-xs leading-relaxed">Hayır, hiçbir fark yoktur. İster tek tek indirin, ister 'Tümünü İndir' butonunu kullanın; sistem aynı işlem motorunu kullanır. Kalite, çözünürlük veya renklerde asla bir kayıp yaşanmaz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><ListChecks size={14} className="text-lime-300" /> HER FOTOĞRAFA FARKLI AYAR YAPARSAM KORUNUR MU?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet, kesinlikle. 20 fotoğraf yükleyip her birine farklı ayar (birine HD, diğerine 3 parça vb.) yapabilirsiniz. 'Tümünü İndir' dediğinizde sistem sırayla her fotoğrafı kendi özel ayarıyla işleyip indirir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Scissors size={14} className="text-rose-300" /> EN İYİ SONUCU ALMAK İÇİN KAÇA BÖLMELİYİM?</h3><p className="text-gray-400 text-xs leading-relaxed">Altın kural: Fotoğrafınızın doğal yapısına (yani içindeki kare sayısına) göre bölün. Örneğin 4 kareli bir Dump fotoğrafınız varsa, en sağlıklı sonuç için 4 parçaya bölün. Eğer 4 kareli fotoğrafı 10'a bölerseniz, parçalar anlamsız ve çok küçük olur.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Files size={14} className="text-indigo-300" /> TOPLU İNDİR İLE TÜMÜNÜ İNDİR ARASINDA FARK VAR MI?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet, kapsam farkı vardır. <strong>"Tümünü İndir"</strong> butonu sadece o an ekranda gördüğünüz 1 fotoğrafın parçalarını indirir. <strong>"Toplu İndir"</strong> ise sol listedeki 10-20 fotoğrafın tamamını, her biri ayrı klasör olacak şekilde tek bir paket (ZIP) halinde indirir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Eraser size={14} className="text-rose-500" /> NEDEN UYGULAMADAN ÇIKINCA VEYA YENİLEYİNCE HER ŞEY SİLİNİYOR?</h3><p className="text-gray-400 text-xs leading-relaxed">Bu bir hata değil, güvenlik tercihidir. Fotoğraflarınız sadece geçici hafızada (RAM) tutulur. Sayfayı yenilediğinizde veya uygulamayı <strong>tamamen kapattığınızda</strong> her şey silinir. Ancak; <strong>uygulamayı kapatmadan</strong> sadece ana ekrana dönerseniz veya başka bir uygulamaya/sekmeye geçip geri dönerseniz <strong>resimleriniz gitmez</strong>, kaldığınız yerden devam edebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Trash2 size={14} className="text-red-400" /> YENİDEN BÖL, SİL VE SIFIRLA NE İŞE YARAR?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>Yeniden Böl:</strong> Mevcut ayarlarla işlemi tekrar başlatır. Olası bir takılma durumunda (bug) sayfayı yenilemeden düzeltmenizi sağlar.<br /><strong>Sil:</strong> Sadece o an seçili olan fotoğrafı siler.<br /><strong>Sıfırla:</strong> Seçili fotoğraf hariç, listedeki diğer tüm fotoğrafları temizler.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Layers size={14} className="text-blue-400" /> YAN TARAFTAKİ 1/20 VE (+) İKONU NE İŞE YARAR?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>1/20:</strong> Sol taraftaki menüde, şu an kaç fotoğraf yüklü olduğunu ve kotanızı gösterir (En fazla 20).<br /><strong>(+) İkonu:</strong> Mevcut listenizi silmeden, üzerine yeni fotoğraflar eklemenizi sağlar. Böylece tek seferde birden çok albümü birleştirip toplu işlem yapabilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Grid size={14} className="text-violet-400" /> PARÇA SAYISI (3, 4, 6, 9) NEDEN FARKLI BÖLÜNÜYOR?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>Çift Sayılar (2, 4, 6, 8, 10):</strong> Sistem bunları genellikle 'Izgara' (Grid) mantığıyla böler. Bu yüzden dikey fotoğraflarda kenarlarda boşluk kalabilir.<br /><strong>Tek Sayılar (3, 5, 7, 9):</strong> Genellikle tek sütun (alt alta) dizilir, boşluk olmaz.<br /><strong>Önemli:</strong> 1 parça seçseniz bile, eğer fotoğrafınız çok ince ve dikeyse, ekrana sığdırmak için yanlarda boşluk görünebilir. Bu tamamen fotoğrafın kendi formatından kaynaklıdır.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Layout size={14} className="text-indigo-400" /> YATAY FOTOĞRAFLARDA NEDEN HİÇ BOŞLUK ÇIKMIYOR?</h3><p className="text-gray-400 text-xs leading-relaxed">Çünkü yatay fotoğraflar doğası gereği geniştir. İster 1 parçaya, ister 10 parçaya bölün; sistem onları her zaman ekranın sağ ve sol kenarlarına tam oturtur. Yatay görsellerde kenar boşluğu (siyah bar) oluşması teknik olarak mümkün değildir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><ZoomOut size={14} className="text-rose-400" /> ZOOM'U %100'DEN AŞAĞI (%90 VB.) YAPINCA NEDEN KENARDA BOŞLUK ÇIKIYOR, İNER Mİ?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>Neden Çıkıyor?</strong> Çünkü fotoğrafı ekrandan daha küçük hale getiriyorsunuz (uzaklaştırıyorsunuz), bu yüzden fotoğrafın kaplamadığı boş alanlarda arka planı (siyah boşluk) görüyorsunuz. Bu tamamen görseldir.<br /><br /><strong>İner mi?</strong> <strong>Kesinlikle HAYIR.</strong> "İNDİR" butonuna bastığınızda sistem o boşlukları yok sayar ve fotoğrafınızı %100 orijinal boyutunda, kenarlarda siyahlık olmadan tertemiz şekilde indirir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><FileImage size={14} className="text-yellow-500" /> HANGİ FORMATI (JPG, PNG, WEBP) SEÇMELİYİM?</h3><p className="text-gray-400 text-xs leading-relaxed"><strong>JPG:</strong> En yaygın format. Paylaşım için ideal ve dengelidir.<br /><strong>PNG:</strong> Kayıpsız kalitedir. Keskinlik çok önemlidir ama dosya boyutu yüksek olabilir.<br /><strong>WEBP:</strong> Hem yüksek kalite hem de düşük boyut sunan yeni nesil formattır. Instagram destekler.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Github size={14} className="text-white" /> BU SİTE AÇIK KAYNAK KODLU MU?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet, Dump Splitter %100 Açık Kaynaklıdır (Open Source). Kodlarımızı şeffaf bir şekilde GitHub üzerinde inceleyebilir, geliştirebilir veya güvenliğini kendiniz test edebilirsiniz.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><ShieldCheck size={14} className="text-green-500" /> BU SİTE LİSANSLI MI?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet, bu proje <strong>GNU Genel Kamu Lisansı v2.0 (GPLv2)</strong> ile korunmaktadır. Bu lisans, yazılımın özgürce kullanılmasına, dağıtılmasına ve değiştirilmesine izin verirken, özgürlüğünün korunmasını da garanti altına alır.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Move size={14} className="text-purple-400" /> ALTTAKİ SİYAH ÇUBUK (BAR) NEDİR? (+), (-) VE İKONLAR NE YAPAR?</h3><p className="text-gray-400 text-xs leading-relaxed">Bu <strong>Kontrol Paneli</strong> size şunları sunar:<br /><strong>Çözünürlük (Örn: 1080x1920):</strong> Fotoğrafınızın o anki çıkış boyutunu gösterir.<br /><strong>(- / +) ve %:</strong> Ekranda fotoğrafı <strong>yakınlaştırıp uzaklaştırmanızı</strong> sağlar. Bu sadece görünümü değiştirir, fotoğrafın boyutunu bozmaz.<br /><strong>Kare İkon:</strong> Yakınlaştırmayı sıfırlar (%100 yapar).<br /><strong>Hareket:</strong> Paneli sağa-sola sürükleyebilirsiniz. Kullanım kolaylığı için alt kısıma sabitlenmiştir.</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><FolderInput size={14} className="text-cyan-500" /> TOPLU FOTOĞRAF VİDEO YÜKLEYEBİLİR / SÜRÜKLEYEBİLİR MİYİM?</h3><p className="text-gray-400 text-xs leading-relaxed">Evet! İsterseniz 'Dosya Yükle' butonuna basıp galerinizden birden fazla fotoğrafı videoyu aynı anda seçebilir, isterseniz de bilgisayardan klasördeki fotoğrafları tutup topluca ekranın içine sürükleyebilirsiniz (Drag & Drop). Sistem hepsini sıraya dizecektir. (Tek seferde en fazla 20 adet yükleyebilirsiniz).</p></div>
          <div className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 h-full"><h3 className="text-white font-bold text-xs uppercase tracking-wider mb-2 flex items-center gap-2"><Timer size={14} className="text-orange-500" /> LOADING EKRANINDA UZUN SÜRE KALIRSAM NE YAPMALIYIM?</h3><p className="text-gray-400 text-xs leading-relaxed">Eğer yükleme ekranı 30 saniyeden uzun sürüyorsa bu genellikle tarayıcı hafızasının (RAM) dolmasından veya yüklenen dosyanın çok yüksek çözünürlüklü (8K/4K) olmasından kaynaklanır.<br /><br /><strong>Çözüm:</strong> Sayfayı yenileyin (F5), tarayıcınızın önbelleğini temizleyin veya daha hafif bir dosya ile tekrar deneyin. Sorun devam ederse farklı bir tarayıcı (Chrome/Safari) kullanmayı deneyebilirsiniz.</p></div>
        </div>
...
const App = () => {
  ...
  const [ultraHd4xMode, setUltraHd4xMode] = useState(DEFAULT_SETTINGS.ultraHd4xMode);
  ...
  const handleSwitchFile = (fileItem) => {
    ...
    setUltraHd4xMode(s.ultraHd4xMode ?? DEFAULT_SETTINGS.ultraHd4xMode);
            ...
            <div className="flex-1 lg:overflow-y-auto custom-scrollbar p-6 space-y-6 lg:space-y-8">
              <div className="space-y-6">
                <div className="p-5 bg-white/[0.03] border border-white/10 rounded-[28px] space-y-5 shadow-inner">
                  ...
                  <div className="space-y-2 relative group cursor-not-allowed" onClick={() => showToast("AI Enhance özelliği bakım çalışması nedeniyle devre dışıdır.", "error")} title="⚠️ Bakım Çalışması: Bu özellik geçici olarak kullanılamıyor">
                  ...
                  {/* HD MODE - MAINTENANCE */}
                  <div className="space-y-2 border-t border-white/5 pt-4 relative group cursor-not-allowed" onClick={() => showToast("HD Kalite özelliği bakım çalışması nedeniyle devre dışıdır.", "error")} title="⚠️ Bakım Çalışması: Bu özellik geçici olarak kullanılamıyor">
                    <div className="opacity-50 grayscale pointer-events-none">
                      <FeatureToggle featureKey="hdMode" state={false} onToggle={() => { }} onInfo={setFeatureInfo} />
                    </div>
                  </div>
                  {/* OPTIMIZE - MAINTENANCE */}
                  <div className="space-y-2 border-t border-white/5 pt-4 relative group cursor-not-allowed" onClick={() => showToast("Optimize özelliği bakım çalışması nedeniyle devre dışıdır.", "error")} title="⚠️ Bakım Çalışması: Bu özellik geçici olarak kullanılamıyor">
                    <div className="opacity-50 grayscale pointer-events-none">
                      <FeatureToggle featureKey="optimize" state={false} onToggle={() => { }} onInfo={setFeatureInfo} />
                    </div>
                  </div>
                  {/* SMART CROP - MAINTENANCE */}
                  <div className="space-y-2 border-t border-white/5 pt-4 relative group cursor-not-allowed" onClick={() => showToast("Smart Crop özelliği bakım çalışması nedeniyle devre dışıdır.", "error")} title="⚠️ Bakım Çalışması: Bu özellik geçici olarak kullanılamıyor">
                    <div className="opacity-50 grayscale pointer-events-none">
                ...
                <div className="p-5 bg-white/[0.03] border border-white/10 rounded-[28px] space-y-5 shadow-inner mt-6">
                  ...
                  <div className="space-y-2 relative group cursor-not-allowed" onClick={() => showToast("Ultra HD (AI SR 2X) özelliği bakım çalışması nedeniyle devre dışıdır.", "error")} title="⚠️ Bakım Çalışması: Bu özellik geçici olarak kullanılamıyor">
                    <div className="opacity-50 grayscale pointer-events-none">
                      <FeatureToggle featureKey="ultraHd" state={false} onToggle={() => { }} onInfo={setFeatureInfo} />
                    </div>
                  </div>
                  <div className="space-y-2 border-t border-white/5 pt-4 relative group cursor-not-allowed" onClick={() => showToast("4X Super Resolution özelliği şu an bakım çalışması nedeniyle devre dışıdır.", "error")} title="⚠️ Bakım Çalışması: Bu özellik geçici olarak kullanılamıyor">
                    <div className="opacity-50 grayscale pointer-events-none">
                  ...
                  <div className={`w-full ${splitCount === 1 ? 'max-w-none px-2 md:px-4' : 'max-w-6xl'} mx-auto space-y-8 md:space-y-16 pb-32 md:pb-40 flex flex-col items-center`}>
                    ...
                    <div className={`grid gap-4 md:gap-8 w-full justify-items-center ${splitCount === 1 ? 'grid-cols-1' : (splitCount % 2 !== 0 || splitCount === 2 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2')}`}>
                      {splitSlides.length > 0 ? splitSlides.map((s) => (
                        <div key={`${uploadedFile}-${s.id}`} style={{ aspectRatio: s.aspectRatio }} className="relative w-full max-w-[500px] h-auto max-h-[50vh] md:max-h-[70vh] group hover:scale-[1.01] transition-all flex items-center justify-center snap-center rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl bg-black animate-in fade-in slide-in-from-bottom-10 duration-700 ease-out">
                          ...
                          {/* OVERLAY - Fixed to Container (Outside Zoom) */}
