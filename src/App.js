    ...
    <div className={`fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 overflow-y-auto transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-[#0f0f0f] border border-white/10 rounded-3xl max-w-3xl w-full p-6 md:p-10 relative shadow-2xl my-auto transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        <div className="flex items-center gap-4 mb-8 border-b border-white/10 pb-6">
          ...
          <div><h2 className="text-xl md:text-2xl font-black text-white uppercase tracking-tighter">Güvenlik ve Gizlilik Protokolü</h2><p className="text-xs text-gray-500 font-medium uppercase tracking-widest mt-1">Son Güncelleme: 17.01.2026 • Final Sürüm 4.7 (Secure Build)</p></div>
        </div>
    ...
    if (!isSilent) {
      ...
      if (!canUseCache) {
        ...
        if (downloadFormat !== prevFormatRef.current) {
          ...
          await new Promise(r => setTimeout(r, 50)); // 350ms -> 50ms (User Request: High Speed)
